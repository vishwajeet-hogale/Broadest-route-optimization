...
def get_callbacks():
    ...
    model_cp = ModelCheckpoint(filepath="./models/bestmodel.h5",monitor="val_accuracy",save_best_only = True,verbose=1)
    return [es,model_cp]
...
def train():
    train_sequences,train_labels,train_num_classes = preprocess_for_model("../data/RoutesData.csv")
    test_sequences,test_labels,num_classes = preprocess_for_model("../data/TestRoutesData.csv")

    model = create_network(train_sequences,train_num_classes + 1)

    # Train the model
    history = model.fit(train_sequences, train_labels, epochs=150, batch_size=16, verbose=1,validation_split = 0.1,callbacks=get_callbacks())
